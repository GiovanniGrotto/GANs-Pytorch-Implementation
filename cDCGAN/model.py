...
class Discriminator(nn.Module):
    def __init__(self, img_channels, channels_d, num_labels, img_size):
        super(Discriminator, self).__init__()
        self.img_size = img_size
        self.disc = nn.Sequential(
            nn.Conv2d(in_channels=img_channels+1, out_channels=channels_d, kernel_size=4, stride=2, padding=1),
            nn.LeakyReLU(0.2),
        ...
        self.embed = nn.Embedding(num_labels, img_size*img_size)

    def forward(self, x, labels):
        labels_embedded = self.embed(labels).view(labels.shape[0], 1, self.img_size, self.img_size)
        x = torch.cat([x, labels_embedded], dim=1)
        return self.disc(x)


class Generator(nn.Module):
    def __init__(self, noise_channels, img_channels, channels_g, num_labels, img_size, embed_size):
        super(Generator, self).__init__()
        self.img_size = img_size
        self.gen = nn.Sequential(
            nn.ConvTranspose2d(in_channels=noise_channels+embed_size, out_channels=channels_g*16, kernel_size=4, stride=1, padding=0),
        ...
        self.embed = nn.Embedding(num_labels, embed_size)

    def forward(self, x, labels):
        labels_embedded = self.embed(labels).unsqueeze(2).unsqueeze(3)
        x = torch.cat([x, labels_embedded], dim=1)
        return self.gen(x)


# This is a better method to obtain a Lipschitz continuous function than weight clipping
def gradient_penalty_fn(discriminator, real, generated, labels):
    ...
    mixed_scores = discriminator(interpolated_imgs, labels)
...
disc = Discriminator(channel_img, channels_d, num_labels, img_size).to(device)
gen = Generator(channel_noise, channel_img, channels_g, num_labels, img_size, embed_size).to(device)
...
for epoch in range(epochs):
    ...
    for batch_idx, (data, label) in enumerate(loop):
        ...
        for _ in range(critic_it):
            ...
            disc_real = disc(data, label).reshape(-1)
            disc_fake = disc(fake, label).reshape(-1)
            gp = gradient_penalty_fn(disc, data, fake, label)
